...
public class Stenfatic {
	...
	// 通过坐标获得方块对象
	public BlockCore loadsblock(String x, String y, String z) {
		byte stepDone = 0;
		Element site = blocks(), site1 = blocks();
		try {// 此处代码的错误来自于对null调用方法，有意为之，无需截获
			stepDone += (site1 = site.element("Y" + y)) == null ? 0 : 1;
			stepDone += (site = site1.element("Z" + z)) == null ? 0 : 1;
			stepDone += (site1 = site.element("X" + x)) == null ? 0 : 1;
		} finally {
			String s = "";// 用于简化描述，但是好像参数的装配顺序未定义？
			switch (stepDone) {
			// 0~2为生成空气，存档中普通空气保存时会被删除
			case (0):
				(site1 = site.addElement("Y" + (s = y), s)).setText(s);
			case (1):
				(site = site1.addElement("Z" + (s = z), s)).setText(s);
			case (2):
				site1 = site.addElement("X" + (s = x), s);
				site1.addAttribute("x", s).addAttribute("id", "air");
			}
		}
	...
	public BlockCore loadblock(int x, int y, int z) {
		return loadsblock(String.valueOf(x), String.valueOf(y), String.valueOf(z));
	}
	...
	// 未组成壳体的环与柱将被移除
	public void delCountry(BlockCountry country) {//do something
	}
}
